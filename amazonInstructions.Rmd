...
&nbsp;
&nbsp;
###7. under configure instance details: select "Terminate" under shutdown behavior. Leave the rest of the options.
###8. leave storage and tag options alone.
###9. select "create a new security group" and http" security group to access AMI via browser (port 80) or ssh (port 22)
...
&nbsp;
###12. Launch instance.
###13. To stop (pause) your instance, go to the "Actions Menu" at the top of the instance list, go down to "instance state" and then hit "stop". Select "start" to resume instance.
&nbsp;
&nbsp;
![](figure5.png)
&nbsp;
&nbsp;
###14. To interact with and run your instance in your browser, scroll down to your "Public DNS", copy and paste the string there, and paste in your browser of choice
###15. the login info is username: rstudio, password: rstudio (if you would like to change the password use passwd(), it will prompt
you to provide the existing password and then type in a new password)
###16. start your work!
###17. to automatically stop your instance when model run is complete:
...
Note: If you use the free tier instance, there is only one core. This means that if you close the tab with your instance, you can not reload it while it is running because it requires a second core for server activity. 
...
```
This bit of code can go at the end of your model to save your model run(s) in your dropbox.
```{r}
...
###4. open PuTTY
###5. host name (specific to the Rstudio server AMI): \S rstudio@public_dns_name
